/* Home page — keeps the original animated hero, adds the rest of the homepage sections. */
...
function Hero() {
  ...
  useEffect(() => {
    ...
    if (!headline) return;
    const intervals = { off: null, rare: [7000, 5000], medium: [3500, 3500], often: [1500, 1500] };
    let timer = 0;
    const flash = () => {
      const freq = window.__opusGlitchFreq || "medium";
      const range = intervals[freq];
      if (!range) {timer = setTimeout(flash, 1500);return;}
      headline.setAttribute("data-flashing", "true");
      setTimeout(() => headline.removeAttribute("data-flashing"), 220);
      timer = setTimeout(flash, range[0] + Math.random() * range[1]);
    };
    timer = setTimeout(flash, 2500);
    ...
    <section className="hero">
      ...
      <div className="hero-figure" ref={figRef}>
        ...
        <div className="img-wrap" ref={imgWrapRef}>
          <img src="assets/hero_main.png" alt="Three figures in motion, rendered with chromatic glitch effect" style={{ objectFit: "fill", padding: "150px 0px 0px" }} />
        </div>
...
// ---------------------------------------------------------------
function HomePage() {
  const { SERVICE_CATEGORIES, CASE_STUDIES, TESTIMONIALS, HOME_FAQS, HOME_STATS } = window.SITE;
...
Object.assign(window, { Hero, HomePage });